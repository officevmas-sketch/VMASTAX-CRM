...
.kpi-grid { display:grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: 18px; margin: 6px 0 18px 0; }
.kpi-card { padding: 22px 24px; border-radius: 22px; background: rgba(255,255,255,.98); border: 1px solid rgba(201,154,24,.22); box-shadow: 0 10px 30px rgba(31,41,55,.08); transition: transform .25s ease, box-shadow .25s ease, border .25s ease; min-height: 132px; width:100%; box-sizing:border-box; overflow:hidden; }
@media (min-width: 1350px) { .kpi-grid { grid-template-columns: repeat(6, minmax(150px, 1fr)); } }
...
.kpi-label { color:#64748B; font-size:.78rem; line-height:1.35; text-transform:uppercase; letter-spacing:.08em; white-space:normal; overflow-wrap:anywhere; }
.kpi-value { font-size:clamp(1.35rem, 2.1vw, 1.9rem); line-height:1.18; font-weight:800; color:#B8860B; margin-top:10px; white-space:normal; overflow-wrap:anywhere; }
.kpi-note { color:#475569; font-size:.85rem; line-height:1.45; margin-top:8px; }
...
if page == "Executive Dashboard" and not filtered.empty:
    st.caption(f"Amount display mode: {amount_unit}. Source values are stored in absolute rupees.")
    ...
    kpi_items = [
        ("Total Clients", total, "Active database"),
        ("Open Cases", open_cases, "Need action"),
        ("Completed", completed, f"{completed/max(total,1):.0%} closure"),
        ("Fees", money(fees), "Billed / expected"),
        ("Received", money(received), "Collection"),
    ...
    with left:
        ...
        fig.update_layout(template="plotly_white", paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)", margin=dict(t=55,l=10,r=10,b=10))
    ...
    with right:
        ...
        fig.update_layout(template="plotly_white", paper_bgcolor="rgba(0,0,0,0)", height=380, margin=dict(t=55,l=10,r=10,b=10))
        st.plotly_chart(fig, use_container_width=True)
        st.markdown("</div>", unsafe_allow_html=True)
    a,b = st.columns(2)
    with a:
        ...
        fig = px.treemap(service, path=["service"], values="fees_display", color="outstanding_display", title=f"Service Portfolio by Fees & Outstanding - {amount_unit}")
        fig.update_layout(template="plotly_white", paper_bgcolor="rgba(0,0,0,0)", margin=dict(t=55,l=10,r=10,b=10))
        st.plotly_chart(fig, use_container_width=True)
    with b:
