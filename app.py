...
def apply_filters(df):
    ...
    if search_text:
        mask = out[["client_name", "business_name", "mobile", "email", "pan_number", "gstin", "tan", "msme_number", "other_registration_number", "service", "assigned_to", "managed_by"]].fillna("").astype(str).agg(" ".join, axis=1).str.contains(search_text, case=False, na=False)
        out = out[mask]
    return out
...
if page == "Executive Dashboard" and not filtered.empty:
    ...
    if not filtered.empty:
        view_cols = ["client_name","business_name","mobile","email","pan_number","gstin","tan","msme_number","other_registration_number","service","financial_year","assigned_to","managed_by","status","priority","fee_amount","amount_received","balance_amount","next_followup_date","remarks"]
