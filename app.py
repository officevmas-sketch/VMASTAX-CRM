...
DEFAULT_COLUMNS = [
    "client_name", "business_name", "mobile", "email", "pan_number", "gstin", "tan", "msme_number", "other_registration_number",
...
def init_db():
    ...
    conn.execute("""
        CREATE TABLE IF NOT EXISTS clients (
            ...
            client_name TEXT NOT NULL, business_name TEXT, mobile TEXT, email TEXT,
    ...
    for col in ["business_name", "pan_number", "gstin", "tan", "msme_number", "other_registration_number", "managed_by"]:
...
def normalize_import(df):
    ...
    mapping = {
        ...
        "business_name": ["business_name", "business", "firm_name", "company_name", "trade_name", "entity_name", "organisation", "organization"],
...
def insert_rows(conn, df):
    ...
    for _, r in df.iterrows():
        ...
        records.append((name, clean_text(r.get("business_name", "")), clean_text(r.get("mobile", "")), clean_text(r.get("email", "")),
    ...
    conn.executemany("""
        ...
        (client_name,business_name,mobile,email,pan_number,gstin,tan,msme_number,other_registration_number,service,financial_year,lead_source,assigned_to,managed_by,status,priority,fee_amount,amount_received,balance_amount,next_followup_date,remarks,created_at,updated_at)
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
    """, records)
    conn.commit()
...
def get_duplicate_report(conn, df):
    ...
    for c in ["client_name", "business_name", "mobile", "email", "pan_number", "gstin", "tan", "msme_number", "service", "financial_year"]:
...
def render_followup_action_table(dataframe, max_rows=50, show_action=True):
    ...
    if dataframe is None or dataframe.empty:
        ...
        return
    cols = ["client_name", "business_name", "mobile", "service", "assigned_to", "managed_by", "status", "priority", "next_followup_date", "balance_amount"]
    view = dataframe.copy().head(max_rows)
    for col in cols:
        if col not in view.columns:
            view[col] = ""
    html = ["<table class='followup-table'>"]
    html.append("<thead><tr><th>Client</th><th>Business Name</th><th>Mobile</th><th>Service</th><th>Assigned To</th><th>Managed By</th><th>Status</th><th>Priority</th><th>Follow-up Date</th><th>Outstanding</th><th>Action</th></tr></thead><tbody>")
    for _, r in view.iterrows():
        ...
        html.append(
            ...
            f"<td>{clean_text(r.get('business_name',''))}</td>"
...
def apply_filters(df):
    ...
    if search_text:
        mask = out[["client_name", "business_name", "mobile", "email", "pan_number", "gstin", "tan", "msme_number", "other_registration_number", "service", "lead_source", "assigned_to", "managed_by"]].fillna("").astype(str).agg(" ".join, axis=1).str.contains(search_text, case=False, na=False)
        out = out[mask]
    return out
...
if page == "Executive Dashboard" and not filtered.empty:
    ...
    with dash_tab1:
        ...
        with c4:
            ...
            st.plotly_chart(fig, use_container_width=True)
            st.markdown("</div>", unsafe_allow_html=True)
        st.markdown("<div class='tile'><div class='tile-title'>Lead Source Dashboard</div>", unsafe_allow_html=True)
        lead_source_df = filtered.copy()
        if "lead_source" not in lead_source_df.columns:
            lead_source_df["lead_source"] = "Not Available"
        lead_source_df["lead_source"] = lead_source_df["lead_source"].fillna("").astype(str).str.strip().replace("", "Not Specified")
        lead_summary = lead_source_df.groupby("lead_source").agg(
            clients=("id", "count"),
            fees=("fee_amount", "sum"),
            received=("amount_received", "sum"),
            outstanding=("balance_amount", "sum")
        ).reset_index().sort_values("clients", ascending=False)
        lead_summary["fees_display"] = lead_summary["fees"].apply(scaled_amount)
        ...
        with l2:
            fig = px.pie(
                ...
                values="fees_display",
                hole=0.55,
                title=f"Fees by Lead Source ({amount_unit})"
            )
            fig.update_layout(
                template="plotly_white",
                height=330,
        ...
        st.dataframe(
            lead_summary[["lead_source", "clients", "fees", "received", "outstanding"]],
    ...
    with dash_tab4:
        ...
        with b:
            ...
            st.dataframe(due[["client_name","business_name","mobile","service","assigned_to","managed_by","status","priority","next_followup_date","balance_amount","remarks"]].head(15), use_container_width=True, hide_index=True)
            st.markdown("</div>", unsafe_allow_html=True)

elif page == "Client Workspace":
    st.subheader("Client Workspace")
    if not filtered.empty:
        view_cols = ["client_name","business_name","mobile","email","pan_number","gstin","tan","msme_number","other_registration_number","service","financial_year","lead_source","assigned_to","managed_by","status","priority","fee_amount","amount_received","balance_amount","next_followup_date","remarks"]
    ...
    with tab_add:
        with st.form("add_client", clear_on_submit=True):
            ...
            business_name = c2.text_input("Business Name")
        ...
        if df_all.empty: st.info("No records available.")
            ...
            with st.form("edit_client"):
                ...
                edit = {
                    ...
                    "business_name": c2.text_input("Business Name", str(row.get("business_name", ""))),
                    "mobile": c3.text_input("Mobile", str(row.get("mobile", ""))),
                    "email": c1.text_input("Email", str(row.get("email", ""))),
