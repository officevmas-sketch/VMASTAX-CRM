...
def init_auth_db(conn):
    ...
    if conn.execute("SELECT COUNT(*) FROM users").fetchone()[0] == 0:
        ...
        conn.execute("""
            INSERT INTO users
            (username, full_name, password_hash, role, is_active, can_dashboard, can_clients, can_pipeline, can_followups, can_add_edit, can_import_export, can_manage_users, created_at, updated_at)
            VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)
        """, ("admin", "VMAS Admin", hash_password("admin123"), "Admin", 1, 1, 1, 1, 1, 1, 1, 1, now, now))
    conn.commit()
...
def load_users(conn):
    return pd.read_sql_query("SELECT id, username, full_name, role, is_active, can_dashboard, can_clients, can_pipeline, can_followups, can_add_edit, can_import_export, can_manage_users, created_at, updated_at FROM users ORDER BY role, username", conn)
...
def create_user(conn, username, full_name, password, role, permissions):
    ...
    conn.execute("""
        ...
        (username, full_name, password_hash, role, is_active, can_dashboard, can_clients, can_pipeline, can_followups, can_add_edit, can_import_export, can_manage_users, created_at, updated_at)
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)
    """, (username.strip(), full_name.strip(), hash_password(password), role, 1, vals["can_dashboard"], vals["can_clients"], vals["can_pipeline"], vals["can_followups"], vals["can_add_edit"], vals["can_import_export"], vals["can_manage_users"], now, now))
    conn.commit()
...
def update_user(conn, user_id, full_name, role, is_active, permissions, new_password=""):
    ...
    if new_password:
        conn.execute("""
            UPDATE users SET full_name=?, role=?, is_active=?, password_hash=?, can_dashboard=?, can_clients=?, can_pipeline=?, can_followups=?, can_add_edit=?, can_import_export=?, can_manage_users=?, updated_at=? WHERE id=?
...
if page == "Executive Dashboard" and not filtered.empty:
    ...
    with tab1:
        st.dataframe(due[["client_name","mobile","service","assigned_to","managed_by","status","priority","next_followup_date","balance_amount","remarks"]], use_container_width=True, hide_index=True)
    ...
    with tab2:
        st.dataframe(upcoming[["client_name","mobile","service","assigned_to","managed_by","status","priority","next_followup_date","balance_amount","remarks"]], use_container_width=True, hide_index=True)
