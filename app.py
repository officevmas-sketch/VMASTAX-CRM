...
:root { --gold:#D4AF37; --gold2:#F7D774; --black:#050505; --panel:#101114; --soft:#1A1B20; --muted:#BFBFBF; }
html, body, [class*="css"] { font-family: 'Inter', sans-serif; }
.stApp { background: radial-gradient(circle at top left, #221a05 0%, #090909 35%, #000 100%); color: #fff; }
[data-testid="stSidebar"] { background: linear-gradient(180deg, #080808 0%, #15100A 100%); border-right: 1px solid rgba(212,175,55,.3); }
[data-testid="stSidebar"] * { color: #f5f0dc !important; }
.block-container { padding-top: 1.2rem; }
.hero-card { padding: 28px 30px; border-radius: 26px; background: linear-gradient(135deg, rgba(212,175,55,.22), rgba(255,255,255,.05)); border: 1px solid rgba(247,215,116,.35); box-shadow: 0 18px 60px rgba(0,0,0,.45); position: relative; overflow: hidden; }
.hero-card:before { content: ''; position:absolute; top:-90px; right:-70px; width:260px; height:260px; background: radial-gradient(circle, rgba(247,215,116,.33), transparent 68%); animation: pulse 4s ease-in-out infinite; }
@keyframes pulse { 0%,100% { transform:scale(.92); opacity:.6;} 50% { transform:scale(1.08); opacity:1;} }
.hero-title { font-size: 2.2rem; font-weight: 800; margin-bottom: 4px; background: linear-gradient(90deg, #fff, #F7D774, #D4AF37); -webkit-background-clip: text; color: transparent; }
.hero-sub { color:#E7E0C8; font-size:1rem; }
.kpi-card { padding: 22px; border-radius: 22px; background: rgba(16,17,20,.88); border: 1px solid rgba(212,175,55,.26); box-shadow: 0 10px 30px rgba(0,0,0,.35); transition: transform .25s ease, border .25s ease; min-height: 118px; }
.kpi-card:hover { transform: translateY(-5px); border: 1px solid rgba(247,215,116,.75); }
.kpi-label { color:#BFBFBF; font-size:.82rem; text-transform:uppercase; letter-spacing:.08em; }
.kpi-value { font-size:1.85rem; font-weight:800; color:#F7D774; margin-top:8px; }
.kpi-note { color:#DDD; font-size:.85rem; margin-top:4px; }
.section-card { padding: 20px; border-radius: 22px; background: rgba(16,17,20,.84); border:1px solid rgba(255,255,255,.09); box-shadow: 0 12px 30px rgba(0,0,0,.25); }
.kanban { padding: 14px; border-radius: 18px; background: rgba(255,255,255,.045); border:1px solid rgba(212,175,55,.18); min-height:145px; }
.kanban h4 { color:#F7D774; margin:0 0 8px 0; }
.chip { display:inline-block; padding:6px 10px; border-radius:999px; border:1px solid rgba(247,215,116,.45); background:rgba(212,175,55,.12); color:#F7D774; margin:3px; font-size:.78rem; }
.stButton>button, .stDownloadButton>button { border-radius: 14px !important; background: linear-gradient(135deg, #D4AF37, #F7D774) !important; color: #070707 !important; border: 0 !important; font-weight: 800 !important; box-shadow:0 8px 20px rgba(212,175,55,.22); }
.stTabs [data-baseweb="tab-list"] { gap: 8px; }
.stTabs [data-baseweb="tab"] { background:rgba(255,255,255,.06); border-radius:14px; padding:10px 16px; color:#EAD98A; }
.stTabs [aria-selected="true"] { background:linear-gradient(135deg,#D4AF37,#F7D774) !important; color:#050505 !important; font-weight:800; }
[data-testid="stMetricValue"] { color: #F7D774; }
hr { border-color: rgba(212,175,55,.2); }
</style>
"""
...
def init_db():
    ...
    conn.execute("""
        CREATE TABLE IF NOT EXISTS clients (
            ...
            client_name TEXT NOT NULL, mobile TEXT, email TEXT, service TEXT, financial_year TEXT,
...
def insert_rows(conn, df):
    ...
    for _, r in df.iterrows():
        ...
        records.append((name, clean_text(r.get("mobile", "")), clean_text(r.get("email", "")), clean_text(r.get("service", "")) or "ITR Filing",
    ...
    conn.executemany("""
        ...
        (client_name,mobile,email,service,financial_year,lead_source,assigned_to,status,priority,fee_amount,amount_received,balance_amount,next_followup_date,remarks,created_at,updated_at)
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
    """, records)
    conn.commit()
...
def apply_filters(df):
    ...
    if search_text:
        mask = out[["client_name", "mobile", "email", "service"]].fillna("").astype(str).agg(" ".join, axis=1).str.contains(search_text, case=False, na=False)
        out = out[mask]
    return out
...
st.markdown("""
<div class='hero-card'>
  <div class='hero-title'>VMAS Premium CRM Command Center</div>
...
if page == "Executive Dashboard" and not filtered.empty:
    ...
    if not filtered.empty:
        view_cols = ["client_name","mobile","email","service","financial_year","status","priority","fee_amount","amount_received","balance_amount","next_followup_date","remarks"]
